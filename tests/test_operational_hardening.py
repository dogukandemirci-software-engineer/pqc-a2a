...
from pqc_a2a import AgentIdentity, ReplayCache, TrustStore, open_envelope, seal
...
def test_trust_store_enforces_pin_revocation_and_reload(tmp_path):
    ...
    assert open_envelope(recipient, sender, envelope, ReplayCache(), trust_store=store) == {"hello": True}

    store.revoke(sender, "compromised")
    with pytest.raises(ValueError, match="trusted|revoked"):
        open_envelope(recipient, sender, envelope, ReplayCache(), trust_store=store)
...
def test_rotation_requires_old_trust_and_revokes_old_key(tmp_path):
    ...
    assert open_envelope(recipient, new, current, ReplayCache(), trust_store=store) == {"key": "new"}
    old_message = seal(old, recipient, {"key": "old"})
    with pytest.raises(ValueError, match="trusted|revoked"):
        open_envelope(recipient, old, old_message, ReplayCache(), trust_store=store)
